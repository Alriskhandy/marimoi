...
/**
 * Tabel roles sudah ada (dengan slug & is_active), jadi migrasi ini hanya
 * menambahkan guard_name ke roles dan membuat tabel permission milik
 * spatie/laravel-permission. Role user yang ada (users.role_id) disalin ke
 * model_has_roles agar pengecekan permission langsung berfungsi.
 */
return new class extends Migration
{
    public function up(): void
    {
        ...
        DB::table('users')
            ...
            ->orderBy('id')
            ->select(['id', 'role_id'])
            ->each(function ($user) use ($tableNames, $columnNames, $pivotRole) {
                DB::table($tableNames['model_has_roles'])->insert([
                    $pivotRole => $user->role_id,
