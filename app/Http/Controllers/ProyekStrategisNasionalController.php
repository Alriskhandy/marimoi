...
class ProyekStrategisNasionalController extends Controller
{
...
{
    $years = ProyekStrategisNasional::select('tahun')
        ->distinct()
        ->orderBy('tahun', 'desc')
        ->pluck('tahun')
        ->toArray();
    ...
    return view('backend.pages.proyek_strategis_nasional.years', compact('years', 'kategoriLayers'));
}
...
public function getAvailableYearsApi()
{
    $years = ProyekStrategisNasional::select('tahun')
        ...
        ->map(function($item) {
            return [
                'year' => $item->tahun,
                'count' => ProyekStrategisNasional::where('tahun', $item->tahun)->count()
            ];
        });
...
public function indexByYear($year)
{
    ...
    $yearExists = ProyekStrategisNasional::where('tahun', $year)->exists();
    ...
    $lokasis = ProyekStrategisNasional::with('kategori')
        ->where('tahun', $year)
        ->get();
    ...
    return view('backend.pages.proyek_strategis_nasional.by_year', compact('lokasis', 'kategoriLayers', 'year', 'statistics'));
}
...
{
    $kategoriLayers = KategoriPSN::with('children')->whereNull('parent_id')->get();
    return view('backend.pages.proyek_strategis_nasional.input-gis', compact('kategoriLayers', 'year'));
}
...
private function storeWithYear(Request $request, $year)
{
    ...
    try {
        ...
        while ($geometry = $reader->fetchRecord()) {
            ...
            // Simpan dengan tahun
            $lokasi = new ProyekStrategisNasional();
        ...
        return redirect()->route('psn.tahun.show', $year)->with('success', $message);
    } catch (\Exception $e) {
...
{
    $query = DB::table('proyek_strategis_nasionals')
        ->join('kategori_psd', 'proyek_strategis_nasionals.kategori_id', '=', 'kategori_psn.id')
        ->select(
            'proyek_strategis_nasionals.id',
            'proyek_strategis_nasionals.kategori_id',
            'proyek_strategis_nasionals.tahun',
            'kategori_psn.nama as kategori',
            'proyek_strategis_nasionals.deskripsi',
            'proyek_strategis_nasionals.dbf_attributes',
            DB::raw('ST_AsGeoJSON(proyek_strategis_nasionals.geom) as geojson')
        )
        ->where('proyek_strategis_nasionals.tahun', $year);

    // Filter kategori
    if ($request->has('kategori') && !empty($request->kategori)) {
        $categories = is_array($request->kategori) ? $request->kategori : [$request->kategori];
        $query->whereIn('kategori_psn.nama', $categories);
    }
    ...
    if ($request->has('search') && !empty($request->search)) {
        ...
        $query->where(function ($q) use ($search) {
            $q->where('kategori_psn.nama', 'ILIKE', "%{$search}%")
              ->orWhere('proyek_strategis_nasionals.deskripsi', 'ILIKE', "%{$search}%")
...
public function getStatisticsByYear($year)
{
    $stats = [
        'year' => $year,
        'total_locations' => DB::table('proyek_strategis_nasionals')->where('tahun', $year)->count(),
        'categories_count' => DB::table('proyek_strategis_nasionals')->where('tahun', $year)->distinct('kategori_id')->count(),
        'categories' => DB::table('proyek_strategis_nasionals')
            ->join('kategori_psd', 'proyek_strategis_nasionals.kategori_id', '=', 'kategori_psn.id')
            ->select('kategori_psn.nama as kategori', DB::raw('COUNT(*) as count'))
            ->where('proyek_strategis_nasionals.tahun', $year)
            ->groupBy('kategori_psn.nama')
            ->orderBy('count', 'desc')
            ->get(),
    ...
    {
        $lokasis = ProyekStrategisNasional::all();
        $kategoriLayers = KategoriPSN::with('children')->whereNull('parent_id')->get();
        return view('backend.pages.proyek_strategis_nasional.index', compact('lokasis','kategoriLayers'));
    }

   public function edit($id)
{
    $lokasi = ProyekStrategisNasional::findOrFail($id); // atau gunakan model binding jika ingin lebih rapi

    $kategoriLayers = KategoriPSN::orderBy('nama')->get();

    return view('backend.pages.proyek_strategis_nasional.edit', compact('lokasi', 'kategoriLayers'));
}
    ...
    public function create()
    {
        // dd('create');
         $kategoriLayers = KategoriPSN::with('children')->whereNull('parent_id')->get();
        return view('backend.pages.proyek_strategis_nasional.input-gis', compact('kategoriLayers'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            while ($geometry = $reader->fetchRecord()) {
                ...
                // Simpan ke database
                // ProyekStrategisNasional::create([
...
public function update(Request $request, $id)
{
    ...
    $validator = Validator::make($request->all(), [
        'kategori_id' => 'required|exists:kategori_psn,id',
    ...
    try {
        $lokasi = ProyekStrategisNasional::find($id);
        if (!$lokasi) {
...
{
    try {
        return ProyekStrategisNasional::create([
