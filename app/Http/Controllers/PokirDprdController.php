...
class PokirDprdController extends Controller
{
    ...
    {
        $pokirDprds = PokirDprd::all();
        $kategoriLayers = KategoriPokirDprd::with('children')->whereNull('parent_id')->get();
        return view('backend.pages.pokir_dprd.data_spasial', compact('pokirDprds','kategoriLayers'));
    }

   public function edit($id)
{
    $lokasi = PokirDprd::findOrFail($id); // atau gunakan model binding jika ingin lebih rapi

    $kategoriLayers = KategoriPokirDprd::orderBy('nama')->get();

    return view('backend.pages.pokir_dprd.edit', compact('lokasi', 'kategoriLayers'));
}
    ...
    {
         $kategoriLayers = KategoriPokirDprd::with('children')->whereNull('parent_id')->get();
        return view('backend.pages.pokir_dprd.input-gis', compact('kategoriLayers'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('pokir-dprd.index')->with('success', $message);
        } catch (\Exception $e) {
   ...
   public function geojson(Request $request)
{
   $query = DB::table('pokir_dprds')
    ...
    if ($request->has('bbox') && !empty($request->bbox)) {
        ...
        if (count($bbox) === 4) {
            $query->whereRaw("ST_Intersects(pokir_dprds.geom, ST_MakeEnvelope(?, ?, ?, ?, 4326))", $bbox);
        }
    }
...
public function update(Request $request, $id)
{
  ...
  private function validateGeometry($geometry)
    ...
    {
        $lokasi = PokirDprd::findOrFail($id);
        $lokasi->delete();

        return redirect()->route('pokir-dprd.index')->with('success', 'Data berhasil dihapus.');
    }

//     public function destroy($id)
// {
//     // Ambil geometri + jenisnya
//     $lokasi = PokirDprd::selectRaw('*, ST_GeometryType(geom) as geom_type')->findOrFail($id);
