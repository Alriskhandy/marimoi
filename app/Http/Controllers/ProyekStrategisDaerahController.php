...
class ProyekStrategisDaerahController extends Controller
{
    ...
    {
        $proyek_strategis_daerahs = ProyekStrategisDaerah::all();
        $kategoriLayers = KategoriPSD::with('children')->whereNull('parent_id')->get();
        return view('backend.pages.proyek_strategis_daerah.index', compact('proyek_strategis_daerahs','kategoriLayers'));
    }
   ...
   public function edit($id)
    ...
    private function processShapefileInput(Request $request)
    {
        ...
        while ($geometry = $reader->fetchRecord()) {
            ...
            // Simpan lokasi
            $this->saveLokasi($request->kategori_id, $description, $cleanDbfData, $processedWkt, $request->tahun);
    ...
    private function processCoordinatesInput(Request $request)
    {
        ...
        foreach ($coordinates as $index => $coord) {
            ...
            // Simpan lokasi
            $this->saveLokasi($request->kategori_id, $description, $dbfAttributes, $wkt, $request->tahun);
    ...
    private function parseKmlContent($kmlContent, $request)
    {
        ...
        foreach ($placemarks as $placemark) {
            ...
            foreach ($geometries as $geometry) {
               ...
               $this->saveLokasi($request->kategori_id, $finalDescription, $dbfAttributes, $geometry, $request->tahun);
    ...
    private function saveLokasi($kategoriId, $description, $dbfAttributes, $wkt, $tahun)
    {
        $lokasi = new ProyekStrategisDaerah();
        $lokasi->kategori_id = $kategoriId;
        $lokasi->tahun = $tahun; // ← ini bagian baru
...
;
    ...
    if ($request->has('bbox') && !empty($request->bbox)) {
        ...
        if (count($bbox) === 4) {
            $query->whereRaw("ST_Intersects(proyek_strategis_daerahs.geom, ST_MakeEnvelope(?, ?, ?, ?, 4326))", $bbox);
        }
    }

    $proyek_strategis_daerahs = $query->get();

    $features = $proyek_strategis_daerahs->map(function ($lokasi) {
    ...
    {
        $values = DB::table('proyek_strategis_daerahs')
    ...
    public function getCategories()
{
    $categories = DB::table('proyek_strategis_daerahs')
        ->join('kategori_psd', 'proyek_strategis_daerahs.kategori_id', '=', 'kategori_psd.id')
        ->select('kategori_psd.id as kategori_id', 'kategori_psd.nama as kategori', DB::raw('COUNT(*) as count'))
        ->groupBy('kategori_psd.id', 'kategori_psd.nama')
        ->orderBy('kategori_psd.nama')
        ->get();
...
{
    $stats = [
        'total_locations' => DB::table('proyek_strategis_daerahs')->count(),
        'categories_count' => DB::table('proyek_strategis_daerahs')->distinct('kategori_id')->count(),
        'categories' => DB::table('proyek_strategis_daerahs')
            ->join('kategori_psd', 'proyek_strategis_daerahs.kategori_id', '=', 'kategori_psd.id')
...
{
    $proyek_strategis_daerahs = DB::table('proyek_strategis_daerahs')
        ->join('kategori_psd', 'proyek_strategis_daerahs.kategori_id', '=', 'kategori_psd.id')
        ->select('proyek_strategis_daerahs.id', 'proyek_strategis_daerahs.kategori_id', 'kategori_psd.nama as kategori', 'proyek_strategis_daerahs.deskripsi', 'proyek_strategis_daerahs.dbf_attributes', DB::raw('ST_AsGeoJSON(geom) as geojson'))
        ->where('kategori_psd.nama', $kategori)
        ->get();
...
public function indexByYear($year)
{
    ...
    $proyek_strategis_daerahs = ProyekStrategisDaerah::with('kategori')
        ->where('tahun', $year)
        ->get();
    ...
    $statistics = [
        'total' => $proyek_strategis_daerahs->count(),
        'categories' => $proyek_strategis_daerahs->groupBy('kategori_id')->map->count(),
        'year' => $year
    ];
    
    return view('backend.pages.proyek_strategis_daerah.by_year', compact('proyek_strategis_daerahs', 'kategoriLayers', 'year', 'statistics'));
}
