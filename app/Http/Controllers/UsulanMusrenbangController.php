...
class UsulanMusrenbangController extends Controller
{
    
    public function index()
    {
        $usulanMusrenbang = UsulanMusrenbang::all();
        $kategoriLayers = KategoriMusrenbang::with('children')->whereNull('parent_id')->get();
        return view('backend.pages.usulan_musrenbang.data_spasial', compact('usulanMusrenbang','kategoriLayers'));
    }

   public function edit($id)
{
    $lokasi = UsulanMusrenbang::findOrFail($id); // atau gunakan model binding jika ingin lebih rapi

    $kategoriLayers = KategoriMusrenbang::orderBy('nama')->get();

    return view('backend.pages.usulan_musrenbang.edit', compact('lokasi', 'kategoriLayers'));
}
    ...
    {
         $kategoriLayers = KategoriMusrenbang::with('children')->whereNull('parent_id')->get();
        return view('backend.pages.usulan_musrenbang.input-gis', compact('kategoriLayers'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('usulan-musrenbang.index')->with('success', $message);
        } catch (\Exception $e) {
...
{
   $query = DB::table('usulan_musrenbangs')
        ->join('kategori_musrenbangs', 'usulan_musrenbangs.kategori_id', '=', 'kategori_musrenbangs.id')
        ->select(
            'usulan_musrenbangs.id',
            'usulan_musrenbangs.kategori_id',
            'kategori_musrenbangs.nama as kategori', // ⬅️ ini sangat penting!
            'usulan_musrenbangs.deskripsi',
            'usulan_musrenbangs.dbf_attributes',
            DB::raw('ST_AsGeoJSON(usulan_musrenbangs.geom) as geojson')
        );

    // Filter kategori
    if ($request->has('kategori') && !empty($request->kategori)) {
        $categories = is_array($request->kategori) ? $request->kategori : [$request->kategori];
        $query->whereIn('kategori_musrenbangs.nama', $categories);
    }
    ...
    if ($request->has('search') && !empty($request->search)) {
        ...
        $query->where(function ($q) use ($search) {
            $q->where('kategori_musrenbangs.nama', 'ILIKE', "%{$search}%")
              ->orWhere('usulan_musrenbangs.deskripsi', 'ILIKE', "%{$search}%")
    ...
    if ($request->has('bbox') && !empty($request->bbox)) {
        ...
        if (count($bbox) === 4) {
            $query->whereRaw("ST_Intersects(usulan_musrenbangs.geom, ST_MakeEnvelope(?, ?, ?, ?, 4326))", $bbox);
        }
    }
    ...
    {
        $values = DB::table('usulan_musrenbangs')
...
{
    $categories = DB::table('usulan_musrenbangs')
        ->join('kategori_musrenbangs', 'usulan_musrenbangs.kategori_id', '=', 'kategori_musrenbangs.id')
        ->select('kategori_musrenbangs.id as kategori_id', 'kategori_musrenbangs.nama as kategori', DB::raw('COUNT(*) as count'))
        ->groupBy('kategori_musrenbangs.id', 'kategori_musrenbangs.nama')
        ->orderBy('kategori_musrenbangs.nama')
        ->get();
...
{
    $stats = [
        'total_locations' => DB::table('usulan_musrenbangs')->count(),
        'categories_count' => DB::table('usulan_musrenbangs')->distinct('kategori_id')->count(),
        'categories' => DB::table('usulan_musrenbangs')
            ->join('kategori_musrenbangs', 'usulan_musrenbangs.kategori_id', '=', 'kategori_musrenbangs.id')
            ->select('kategori_musrenbangs.nama as kategori', DB::raw('COUNT(*) as count'))
            ->groupBy('kategori_musrenbangs.nama')
            ->orderBy('count', 'desc')
            ->get(),
        'bounds' => DB::table('usulan_musrenbangs')
            ->select(
...
{
    $lokasis = DB::table('usulan_musrenbangs')
        ->join('kategori_musrenbangs', 'usulan_musrenbangs.kategori_id', '=', 'kategori_musrenbangs.id')
        ->select('usulan_musrenbangs.id', 'usulan_musrenbangs.kategori_id', 'kategori_musrenbangs.nama as kategori', 'usulan_musrenbangs.deskripsi', 'usulan_musrenbangs.dbf_attributes', DB::raw('ST_AsGeoJSON(geom) as geojson'))
        ->where('kategori_musrenbangs.nama', $kategori)
        ->get();
...
public function update(Request $request, $id)
{
    $validator = Validator::make($request->all(), [
        'kategori' => 'required|exists:kategori_musrenbangs,id',
    ...
    try {
        $lokasi = UsulanMusrenbang::find($id);
        if (!$lokasi) {
  ...
  private function validateGeometry($geometry)
    ...
    {
        $lokasi = UsulanMusrenbang::findOrFail($id);
        $lokasi->delete();

        return redirect()->route('usulan-musrenbang.index')->with('success', 'Data berhasil dihapus.');
    }

//     public function destroy($id)
// {
//     // Ambil geometri + jenisnya
//     $lokasi = UsulanMusrenbang::selectRaw('*, ST_GeometryType(geom) as geom_type')->findOrFail($id);
...
{
    try {
        return UsulanMusrenbang::create([
