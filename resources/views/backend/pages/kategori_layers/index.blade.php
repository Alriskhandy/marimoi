@extends('backend.partials.main', ['title' => 'Kategori Layer'])
                        ...
                        <div class="col-md-3 stretch-card grid-margin">
                            <div class="card bg-gradient-primary card-img-holder text-white">
                                <div class="card-body">
                                    ...
                                    <h2 class="mb-5">{{ $kategoriLayers->where('parent_id', null)->count() }}</h2>
                        ...
                        <div class="col-md-3 stretch-card grid-margin">
                            <div class="card bg-gradient-info card-img-holder text-white">
                                <div class="card-body">
                                    ...
                                    <h2 class="mb-5">{{ $kategoriLayers->sum('layers_count') ?? 0 }}</h2>
                                    <h6 class="card-text">Akumulasi layer</h6>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-3 stretch-card grid-margin">
                            <div class="card bg-gradient-warning card-img-holder text-white">
                                <div class="card-body">
                                    ...
                                    <h2 class="mb-5">{{ $kategoriLayers->where('is_marker', true)->count() }}</h2>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover" id="kategoriTable">
                            ...
                            <tbody>
                                @php
                                    $no = 1;
                                    $parentKategoris = $kategoriLayers->where('parent_id', null);
                                    $childKategoris = $kategoriLayers
                                            ...
                                            <tr data-id="{{ $child->id }}" class="table-secondary">
                                                ...
                                                <td>
                                                    <div class="btn-group" role="group">
                                                        ...
                                                        <form action="{{ route('kategori-layers.destroy', $child->id) }}"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select class="form-control" id="edit_parent_id" name="parent_id">
                                <option value="">-- Pilih Parent (Opsional) --</option>
                                @foreach ($kategoriLayers->where('parent_id', null) as $parent)
...
@section('scripts')
    <script>
        $(document).ready(function() {
            ...
            window.confirmDelete = function(nama, layerCount) {
                if (layerCount > 0) {
                    alert(
                        `Tidak dapat menghapus kategori "${nama}" karena masih memiliki ${layerCount} layer.`
                    );
                    return false;
                }
            ...
            function loadParentCategories(selectElement, excludeId = null) {
                const parentKategoris = @json($kategoriLayers->where('parent_id', null));
                selectElement.empty();
            ...
            $(document).on('click', '.btn-edit', function() {
                ...
                // Get data via AJAX
                $.get(`{{ route('kategori-layers.index') }}/${id}`, function(response) {
                    if (response.success) {
                        const data = response.data;

                        $('#edit_id').val(data.id);
                        $('#edit_nama').val(data.nama);
                        $('#edit_warna').val(data.warna || '#007bff');
                        $('#edit_deskripsi').val(data.deskripsi);
                        $('#edit_colorPreview').css('background-color', data.warna || '#007bff');
                        ...
                        if (data.is_marker) {
                            ...
                            if (data.icon) {
                                const fa4Icon = convertFa6ToFa4(data.icon);
                                $('#edit_iconPreview').html(`
                                    <div class="d-flex align-items-center">
                                        <i class="${fa4Icon} fa-2x me-3" style="color: #007bff;"></i>
                                        <div>
                                            <div><strong>Preview:</strong> <span class="text-muted">(FA4 untuk tampilan)</span></div>
                                            <div><small><strong>Disimpan:</strong> <code>${data.icon}</code></small></div>
                                        </div>
                                    </div>
                                `);
                            }
                        ...
                        // Load parent categories (exclude current category)
                        loadParentCategories($('#edit_parent_id'), data.id);
                    ...
                    // Fallback: use data attributes if AJAX fails
                    const nama = $(this).data('nama');
                    $('#edit_id').val(id);
                    $('#edit_nama').val(nama);
                    $('#edit_warna').val($(this).data('warna') || '#007bff');
                    $('#edit_deskripsi').val($(this).data('deskripsi'));
                    $('#edit_colorPreview').css('background-color', $(this).data('warna') ||
                        '#007bff');
                    ...
                    if ($(this).data('is-marker')) {
                        ...
                        if ($(this).data('icon')) {
                            const fa4Icon = convertFa6ToFa4($(this).data('icon'));
                            $('#edit_iconPreview').html(`
                                <div class="d-flex align-items-center">
                                    <i class="${fa4Icon} fa-2x me-3" style="color: #007bff;"></i>
                                    <div>
                                        <div><strong>Preview:</strong> <span class="text-muted">(FA4 untuk tampilan)</span></div>
                                        <div><small><strong>Disimpan:</strong> <code>${$(this).data('icon')}</code></small></div>
                                    </div>
                                </div>
                            `);
                        }
                    ...
                    if ($(this).data('parent-id')) {
                        setTimeout(() => {
                            $('#edit_parent_id').val($(this).data('parent-id'));
                        }, 100);
                    }
                    ...
                    if (data.success) {
                        ...
                        if (kategori.children && kategori.children.length > 0) {
                            ...
                            $.each(kategori.children, function(index, child) {
                                ...
                                childrenHtml +=
                                    `<span class="badge badge-info me-1">${childType} ${child.nama}</span>`;
                            });
            ...
            if ($('#kategoriTable tbody tr').length > 10) {
                $('#kategoriTable').DataTable({
                    ...
                    columnDefs: [{
                        orderable: false,
                        targets: [2, 5] // Disable sorting on Warna and Aksi columns
                    }]
                });
            }
