        ...
        function updateLayerControl(categories) {
            ...
            categories.forEach(cat => {
                const count = categoryLayers[cat.kategori] ? categoryLayers[cat.kategori].length : 0;
        ...
        function loadDbfColumns() {
            fetch('/api/dbf/columns')
                ...
                .catch(err => {
                    ...
                    document.getElementById('dbf-filters').innerHTML = '<p class="text-muted small">Tidak ada filter tambahan tersedia</p>';
                });
        }
        ...
        function createDbfFilters() {
            ...
            dbfColumns.slice(0, 3).forEach(column => { // Batasi 3 kolom untuk UI yang lebih clean
                html += `
            ...
            fetch(`/api/dbf/column/${column}/values`)
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (select) {
                            data.values.slice(0, 20).forEach(value => { // Batasi 20 nilai pertama
                                const option = document.createElement('option');
                                option.value = value;
                                option.textContent = value.length > 30 ? value.substring(0, 30) + '...' : value;
                                select.appendChild(option);
                            });
                        }
                    }
                })
                .catch(err => console.error(`Gagal memuat nilai untuk ${column}:`, err));
        }
            ...
            fetch(url)
                ...
                .then(data => {
                    if (!data || !data.features || data.features.length === 0) {
                        showAlert("Tidak ada data yang ditemukan dengan filter yang dipilih.", "info");
                        updateLayerControl([]);
                    ...
                    // Reset data
                    Object.keys(categoryLayers).forEach(key => delete categoryLayers[key]);
                    Object.keys(layerGroups).forEach(key => delete layerGroups[key]);

                    // Kelompokkan data berdasarkan kategori
                    data.features.forEach(feature => {
                        const kategori = feature.properties.kategori || 'Tidak Dikategorikan';
                            ...
                            const layer = L.geoJSON(feature, {
                                ...
                                onEachFeature: function(feature, layer) {
                                    ...
                                    let popupContent = `
                                        <div class="popup-custom">
                                            <div class="popup-header">
                                                ...
                                                ${props.kategori || 'Tidak Dikategorikan'}
                                            </div>
                                    `;

                                    if (props.deskripsi && props.deskripsi !== '-' && props.deskripsi !== '') {
                                        popupContent += `
                                    ...
                                    if (dbfAttributes.length > 0) {
                                        ...
                                        dbfAttributes.slice(0, 6).forEach(key => { // Batasi 6 atribut untuk UI yang clean
                                        ...
                                        if (dbfAttributes.length > 6) {
                                            popupContent += `<small class="text-muted">... dan ${dbfAttributes.length - 6} atribut lainnya</small>`;
                                        }
        ...
        map.on('moveend', function() {
            ...
            const newUrl = `${window.location.pathname}#${zoom}/${center.lat.toFixed(5)}/${center.lng.toFixed(5)}`;
