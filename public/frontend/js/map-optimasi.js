...
async function loadCategoryData(categoryName, parentName = null, grandparentName = null) {
    ...
    try {
        ...
        // --- CACHE CHECK: gunakan cache jika ada dan masih valid ---
        const cacheKey = makeCategoryCacheKey(grandparentName, parentName, categoryName);
        const cached = getCachedGeoJSON(cacheKey);
        if (cached && Array.isArray(cached.features) && cached.features.length > 0) {
            // gunakan cached data langsung tanpa fetch
            L.geoJSON(cached, {
                pointToLayer: (feature, latlng) => {
                    // markerOptions tidak disimpan sepenuhnya di cache; gunakan warna/icon dari kategori metadata jika tersedia
            ...
            updateLoadingProgress(cached.features.length, cached.features.length, "Selesai (dari cache)");
            // jika checkbox aktif di UI, tambahkan layer ke map
            // (penanganan penambahan map dilakukan di tempat pemanggil checkbox)
            hideLoadingOverlay();
            updateCheckboxLoadingState(categoryName, false);
            if (loadingToast) hideToast(loadingToast);
            showAlert(`Data ${categoryName} dimuat dari cache (${cached.features.length} fitur)`, "success");
            return;
        }
        // --- end cache check ---
        ...
        // buffer untuk mengumpulkan fitur agar bisa di-cache setelah selesai
        ...
        while (hasMore && totalLoaded < maxRecords) {
            try {
                ...
                // append to collected features for caching
                if (geoJsonData?.features?.length) {
                    collectedFeatures.features.push(...geoJsonData.features);
                }

                // existing logic to render features...
        ...
        // Simpan ke cache (jika ada fitur)
        if (collectedFeatures.features.length > 0) {
            try {
                setCachedGeoJSON(cacheKey, collectedFeatures);
            } catch (e) {
                if (window.location.hostname === 'localhost') console.warn("Cache set failed", e);
            }
        }
        ...
        // simpan state setelah layer berhasil dimuat
        saveMapState();
...
// ======= BEGIN ADDED: persistent storage & cache helpers =======
const STORAGE_KEYS = {
    MAP_STATE: "marimoi_map_state_v1",
    CATEGORY_CACHE_PREFIX: "marimoi_cat_cache_v1::", // followed by category key
};

const CATEGORY_CACHE_TTL_MS = 24 * 60 * 60 * 1000; // 24 jam

function makeCategoryCacheKey(grandparent, parent, category) {
    return `${grandparent || ""}::${parent || ""}::${category}`.replace(/\s+/g, "_");
}

function setCachedGeoJSON(cacheKey, data) {
    try {
        const payload = {
            ts: Date.now(),
            data,
        };
        localStorage.setItem(STORAGE_KEYS.CATEGORY_CACHE_PREFIX + cacheKey, JSON.stringify(payload));
    } catch (e) {
        // jika localStorage penuh atau error, ignore silently
        if (window.location.hostname === "localhost") console.warn("Cache write failed", e);
    }
}

function getCachedGeoJSON(cacheKey, ttl = CATEGORY_CACHE_TTL_MS) {
    try {
        const raw = localStorage.getItem(STORAGE_KEYS.CATEGORY_CACHE_PREFIX + cacheKey);
        if (!raw) return null;
        const payload = JSON.parse(raw);
        if (!payload || !payload.ts || !payload.data) return null;
        if (Date.now() - payload.ts > ttl) {
            localStorage.removeItem(STORAGE_KEYS.CATEGORY_CACHE_PREFIX + cacheKey);
            return null;
        }
        return payload.data;
    } catch (e) {
        if (window.location.hostname === "localhost") console.warn("Cache read failed", e);
        return null;
    }
}
...
function saveMapState() {
    try {
        const activeLayers = []; // array of {grandparent,parent,category}
        document.querySelectorAll('#layer-list input[type="checkbox"]').forEach(cb => {
            if (cb.checked) {
                activeLayers.push({
                    category: cb.getAttribute('data-category') || "",
                    parent: cb.getAttribute('data-parent') || "",
                    grandparent: cb.getAttribute('data-grandparent') || ""
                });
            }
        });

        const state = {
            center: map.getCenter ? [map.getCenter().lat, map.getCenter().lng] : mapConfig.center,
            zoom: map.getZoom ? map.getZoom() : mapConfig.zoom,
            basemap: currentBaseMap ? currentBaseMap._url || currentBaseMap.options?.attribution : null,
            activeLayers,
            ts: Date.now()
        };
        localStorage.setItem(STORAGE_KEYS.MAP_STATE, JSON.stringify(state));
    } catch (e) {
        if (window.location.hostname === "localhost") console.warn("saveMapState failed", e);
    }
}
...
function loadMapState() {
    try {
        ...
        if (window.location.hostname === "localhost") console.warn("loadMapState failed", e);
        return null;
    }
}
// ======= END ADDED =======
...
// ======= BEGIN ADDED: restore active layers & map view =======
async function restoreActiveLayersAndViewAfterUIReady() {
    ...
    // restore basemap if stored (try to match by id if possible)
    if (state.basemap && typeof changeBaseMap === "function") {
        // simplest approach: do nothing if current set; otherwise try to find baseMap id that matches preview url / label
        // prefer explicit restoration by saving baseMap id in future
    }

    // restore active layers (toggle checkboxes). We'll try to find checkbox and trigger change
    if (Array.isArray(state.activeLayers) && state.activeLayers.length > 0) {
        for (const item of state.activeLayers) {
            const cat = item.category;
            if (!cat) continue;
            const checkbox = findCheckboxForCategory(cat);
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Try restore map view earlier (if any)
    const saved = loadMapState();
    if (saved && saved.center && saved.zoom !== undefined) {
        try {
            map.setView([saved.center[0], saved.center[1]], saved.zoom);
        } catch (e) {}
    }
    ...
    try {
        ...
        // After UI ready, restore active layers & view
        setTimeout(async () => {
            await restoreActiveLayersAndViewAfterUIReady();
        }, 800);
